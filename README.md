...
6) Outputs
   - FastQC reports: `results/qc/fastqc/<sample>/`
   ...
   - Optional Auspice JSON when `augur.enable: true`: `results/nextstrain/auspice/sarscov2_fourth_wave.json`
...
### NCBI usage note
Set a contact email once per shell for E-utilities. An optional API key improves rate limits.
```bash
...
params:
  ...
  # max_date: 2021-09-30  # optional cap for time-sliced Nextstrain builds
```

## Nextstrain build
You can produce an Auspice JSON for interactive viewing.

1. Prepare `data-private/metadata.tsv` with columns: `strain`, `date`, `region`, `division`, `location`. `strain` must match the FASTA headers.
2. Set `augur.enable: true` in `config/config.yaml`.
3. Run:
   ```bash
   make nextstrain
   ```
...
- O'Toole A, et al. 2021. Pangolin. Virus Evol 7:veab064.
